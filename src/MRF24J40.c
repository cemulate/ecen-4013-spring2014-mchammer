 ...
 * Modified for PIC24 and mplabC30 by Ruben Jönsson 2012.
*/

#define _IN_MRF24J40_
...
static UINT64	MRF24J40Timer=0;

// Call this from a timer interrupt every 256us.
// Used for timeouts
...
{	
	return MRF24J40Timer-savedTimer;
}	
...
BOOL RadioInit(void)					// cold start radio init
{
	...
	RADIO_INTPOL=1;					// Interrupt on negative edge.
	if (RADIO_INT_PIN==0){
		RADIO_IF=1;
	} else {
		RADIO_IF=0;
	...
	RADIO_IE = 1;					// enable radio interrupts
...
//	wReg = toTXfifo(2,BYTEPTR(Tx)+1,2+1);					// frame control (2) + sequence number (1) 
	wReg = toTXfifo(2,BYTEPTR(Tx.frameControl),2);
	wReg = toTXfifo(wReg,BYTEPTR(Tx.frameNumber),1);

	if (Tx.frameControl.dstAddrMode == SHORT_ADDR_FIELD){			// if a short dest addr is present
		wReg = toTXfifo(wReg,BYTEPTR(Tx.dstPANID), 2);			// write dstPANID
		wReg = toTXfifo(wReg,BYTEPTR(Tx.dstAddr), 2);			// write short address
	} else if (Tx.frameControl.dstAddrMode == LONG_ADDR_FIELD){		// if a long dest addr is present
		wReg = toTXfifo(wReg,BYTEPTR(Tx.dstPANID), 2);			// write dstPANID
	...
	if ( Tx.frameControl.srcAddrMode != NO_ADDR_FIELD && 			// if source present
		 ...
		 !Tx.frameControl.panIDcomp ){					// and no PANID compression
		wReg = toTXfifo(wReg,BYTEPTR(Tx.srcPANID), 2);			// then write src PANID
	}
		
	if (Tx.frameControl.srcAddrMode == SHORT_ADDR_FIELD){			// if a short src addr is present
		wReg = toTXfifo(wReg,BYTEPTR(Tx.srcAddr), 2);
	} else if (Tx.frameControl.srcAddrMode == LONG_ADDR_FIELD){		// if a long src addr is present
	...
	lowWrite(WRITE_TXNMTRIG, Tx.frameControl.ackRequest << 2 | 1);		// kick off transmit with above parameters
	RadioStatus.LastTXTriggerTick = MRF24J40Timer;				// record time (used to check for locked-up radio or PLL loss)
}
...
void RadioTXPacket(void)
{
	...
	while(RadioStatus.TX_BUSY){						// If TX is busy, wait for it to clear (for a resaonable time)
		if (timerTicksSince(RadioStatus.LastTXTriggerTick) > MRF24J40_TIMEOUT_TICKS ){
...
{
	while(RadioStatus.TX_BUSY){						// If TX is busy, wait for it to clear (for a resaonable time)
		if ( timerTicksSince(RadioStatus.LastTXTriggerTick) > MRF24J40_TIMEOUT_TICKS ){		// if not ready in a resonable time
			initMRF24J40();									// reset radio hardware (stay on same channel)
		}
	}
...
UINT8 RadioRXPacket(void)
{
	...
	if( Rx.frameControl.securityEnabled )					// if security enabled, toss it (not supported)
	{
		RadioStatus.RXSecurityEnabled++;				// log error
		RadioDiscardPacket();
		return RadioRXPacket();						// yes I know it's a little recursive, but the RXBuffer is
                                                                                // small enough that the stack is unlikely to overflow
        }

	if (Rx.frameControl.frameType == PACKET_TYPE_ACK){			// no PANID present on ACK frames [802.15.4 weakness: No way to know if this ACK is really for you]
		readPoint -= 2;
	}	
	...
	if (Rx.frameControl.srcAddrMode == SHORT_ADDR_FIELD){			// if a short src addr is present
		readPoint = readBytes(BYTEPTR(Rx.srcAddr),readPoint, 2);
	} else if (Rx.frameControl.srcAddrMode == LONG_ADDR_FIELD){		// if a long src addr is present
...
#elif (RADIO_INT==1)
void __attribute__((interrupt,shadow,auto_psv)) _INT1Interrupt(void)
#elif (RADIO_INT==2)
